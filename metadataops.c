 ...
 * Postcondtion: all node memory, if any, is returned to OS,
 ...
 * Postcondtion: displays all op codes in list
 * Exceptions: none
 * Notes: none
 */
 ...
 * Algorithm: acquires the metadata items from file, stores them in a linked
 *            list
 * Precondition: provided file name
 * Postcondition: returns success (Boolean), returns linked list head via
 *                parameter, returns endstate/error message via parameter
         ...
         // check for device cycle time
           // function: compareString
         else if ( compareString( inData->command, "dev") == STR_EQ)
            {
               ...
               //check for faiiled number access
               if( numBuffer <= BAD_ARG_VAL )
                 {
                   //set failure flag
                   arg2FailureFlag = True;
...
* Algortihm: check string argument for one of the allowed commands
* Precondition: input string is provided
* Postcondition: in correct operation,
