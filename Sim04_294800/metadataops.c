    ...
    while( localPtr != NULL )
       {
        ...
        // othewise assume other than dev
        else
           {
            // print NA
               // function: printf
        ...
        // print first int argument
           // function: printf
        printf( "\n\t /arg 2: %d", localPtr->intArg2 );

        // print second int argument
           // function: printf
        printf( "\n\t /arg 3: %d", localPtr->intArg3 );
    ...
    *opCodeDataHead = NULL;
    ...
    if( !getStringToDelimiter( fileAccessPtr, COLON, dataBuffer )
                                    ...
                                    "Start Program Meta-Data Code" ) != STR_EQ )
       {
        ...
        // set end state message
           // function: copyString
        copyString( endStateMsg, "Corrupt meta data leader line error" );
    ...
    if( accessResult == LAST_OPCMD_FOUND_MSG )
       {
        ...
        if( startCount == endCount )
           {
            // add the last node to the linked lsit
               // function: addNode
            localHeadPtr = addNode( localHeadPtr, newNodePtr );
            ...
            // check last line for incorrect end descriptor
               // function: getStringToDelimiter, compareSring
    ...
    // check for any erros found (not no error)
...
OpCodeMessages getOpCommand( FILE *filePtr, OpCodeType *inData )
   {
    ...
    // set all struct values that may not be initialized
    inData->pid = 0;
