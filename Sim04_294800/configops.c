...
bool getConfigData( const char *fileName,
                                ConfigDataType **configData, char *endStateMsg )
   {
    // initialize function/variables
       ...
       int inData, dataLineCode, lineCtr = 0;
       double doubleData;

    // set endStateMsg to success
       // function: copyString
    char testMsg[] = "Configuration file upload succcessful";
        ...
        if( dataLineCode != CFG_CORRUPT_PROMPT_ERR )
           {
            ...
            // otherwise, assume integer data
            else
               {
                // get integer input
                   // function: fscanf
                fscanf( fileAccessPtr, "%d", &inData );
               }

        // check for data value in range
           // function: valueInRange
        if( valueInRange( dataLineCode, inData,
            ...
            switch( dataLineCode )
               {
                ...
                case CFG_MEM_AVAILABLE_CODE:
                   ...
                   tempData->memAvailable = inData;
                   break;

                case CFG_PROC_CYCLES_CODE:

                   tempData->procCycleRate = inData;
                   break;

                case CFG_IO_CYCLES_CODE:

                   tempData->ioCycleRate = inData;
                   break;
