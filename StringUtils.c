 ...
 *            if twoStrings are identical up to the point that one is longer,
 ...
 * Algorithm: concatenates or appends contents of one string onto
 *            the end of another
 ...
 *                source string appended or concatenated to the end of it
 * Exceptions: none
 * Note: limit test loop to maximum characters for safety
 */
...
* Precondtion: given a C-style test string, having a null character ('\0')
...
int getLineTo( FILE *filePtr, int bufferSize, char stopChar,
                                                 ...
                                                 Boolean stopAtNonPrintable )
   {
     // initial function/variables
         ...
         // initialize status reutrn to NO_ERR
      ...
      while( charAsInt != (int) stopChar && bufferSizeAvailable == True)
        {
         ...
         //StopAtNonPrintable breaks out of the loop
         else if ( stopAtNonPrintable == True )
           {
                break;
           }
 ...
 * Algorithm: tests character parameter for upper case, changes it
 *            to lower case, makes no changes if not upper case
...
* Postcondition: all uppercase letters in given string are set
